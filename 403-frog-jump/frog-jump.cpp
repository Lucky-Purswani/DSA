class Solution {
public:
    bool solve(vector<int>& stones, int curr, int k, unordered_map<int, int> &mp, vector<vector<int>> &dp){
        if(curr == stones.size()-1) return true;
        if(dp[curr][k] != -1) return dp[curr][k]; 
        //k-1
        bool km1 = false;
        if(k-1 > 0 && mp.count(stones[curr]+k-1)){
            km1 = solve(stones, mp[stones[curr]+k-1], k-1, mp, dp);
        }
        //k
        bool kk = false;
        if(k > 0 && mp.count(stones[curr]+k)){
            kk = solve(stones, mp[stones[curr]+k], k, mp, dp);
        }
        //k+1
        bool k1 = false;
        if(k+1 > 0 && mp.count(stones[curr]+k+1)){
            k1 = solve(stones, mp[stones[curr]+k+1], k+1, mp, dp);
        }

        return dp[curr][k] = k1 || kk || km1;
